...
def admin_dashboard_panel(request):
    ...
    pending_exit_items = ExitRequest.objects.select_related('employee').filter(
    ...
    pending_work_items = [
    ...
    ] + [
        {
            'type': 'Exit Request',
            'employee': item.employee,
            'title': item.last_working_day,
            'details': item.reason,
            'status': item.status,
            'status_label': item.get_status_display(),
        }
        for item in pending_exit_items
    ]
    context = {
        'summary': {
...
class EmployeeLeaveRequestsView(APIView):
    ...
    def post(self, request):
        ...
        if not LeaveType.objects.filter(name=leave_type_name, is_active=True).exists():
            return Response(
                {'leave_type': 'Select a valid active leave type.'},
                status=status.HTTP_400_BAD_REQUEST,
            )
                ...
                'modules': [
                    {
                        ...
                        'api_path': '/api/admin/hr/departments/',
                    },
                    {
                        'key': 'designations',
                        'label': 'Designation Management',
                        'count': designation_count,
                        'api_path': '/api/admin/hr/designations/',
                    },
                    {
                        'key': 'exit_requests',
                        'label': 'Exit Management',
                        'count': pending_exit_count,
                        'api_path': '/api/admin/hr/exit-requests/',
                    },
                    {
                        'key': 'documents',
                        'label': 'Document Management',
                        'count': document_count,
                        'api_path': '/api/admin/hr/documents/',
                    },
                ],
            },
            'pending_requests': {
...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {
                    'detail': 'Unable to send OTP email. Check your Gmail SMTP credentials and App Password.',
                    'error': str(exc),
                },
