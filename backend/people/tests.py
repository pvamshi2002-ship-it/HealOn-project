...
class EmailSettingsTests(TestCase):
    def test_settings_loads_smtp_credentials_from_repo_root_env_file(self):
        ...
        try:
            ...
            env_path.write_text(
                'EMAIL_HOST_USER=repo.sender@gmail.com\n'
                'EMAIL_HOST_PASSWORD=repo-app-password\n',
                encoding='utf-8',
            )
            import healon_backend.settings as settings_module
            importlib.reload(settings_module)
            self.assertEqual(settings_module.EMAIL_HOST_USER, 'repo.sender@gmail.com')
            self.assertEqual(settings_module.EMAIL_HOST_PASSWORD, 'repo-app-password')
        finally:
...
class DashboardLoginTests(APITestCase):
    ...
    def test_admin_dashboard_includes_hr_admin_modules(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.admin_token.key}')
        response = self.client.get('/api/admin/dashboard/')

        self.assertEqual(response.status_code, 200)
        hr_admin = response.data['hr_admin']
        self.assertIn('departments', hr_admin)
        self.assertIn('designations', hr_admin)
        self.assertIn('pending_exits', hr_admin)
        self.assertIn('documents', hr_admin)
        module_keys = {item['key'] for item in hr_admin['modules']}
        self.assertEqual(
            module_keys,
            {'departments', 'designations', 'exit_requests', 'documents'},
        )

    def test_hr_can_access_hr_admin_module_apis(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.hr_token.key}')

        for path in (
            '/api/admin/hr/departments/',
            '/api/admin/hr/designations/',
            '/api/admin/hr/exit-requests/',
            '/api/admin/hr/documents/',
        ):
            response = self.client.get(path)
            self.assertEqual(response.status_code, 200, msg=path)

    def test_employee_cannot_access_hr_admin_module_apis(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.employee_token.key}')
        response = self.client.get('/api/admin/hr/departments/')
        self.assertEqual(response.status_code, 403)

    def test_hr_can_create_department_and_designation(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.hr_token.key}')
        ...
        department_id = department_response.data['department']['id']

        designation_response = self.client.post(
            '/api/admin/hr/designations/',
            {
                'name': 'QA Analyst',
                'code': 'QA-AN',
        ...
        self.assertEqual(designation_response.status_code, 201)
        self.assertEqual(
            designation_response.data['designation']['department_name'],
            'Quality Assurance',
        )

    def test_public_job_apply_without_auth(self):
        from people.models import Candidate, JobOpening

        job = JobOpening.objects.create(title='Staff Nurse', status=JobOpening.STATUS_OPEN)
        response =         self.client.post(
            f'/api/public/jobs/{job.public_slug}/apply/',
            {
                'full_name': 'Jane Applicant',
                'email': 'jane.applicant@healon.local',
                'phone': '9999999999',
                'experience': '3 years in nursing',
                'skills': 'Patient care, ICU',
                'cover_letter': 'Interested in the role.',
                'resume_file_name': 'resume.pdf',
                'resume_data': 'data:application/pdf;base64,JVBERi0xLjQ=',
            },
            format='json',
        )
        self.assertEqual(response.status_code, 201)
        self.assertTrue(Candidate.objects.filter(email='jane.applicant@healon.local').exists())

    def test_employee_can_view_jobs_and_submit_resignation(self):
        from people.models import ExitRequest, JobOpening

        JobOpening.objects.create(title='Operations Lead', status=JobOpening.STATUS_OPEN)
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.employee_token.key}')

        jobs_response = self.client.get('/api/employee/jobs/')
        self.assertEqual(jobs_response.status_code, 200)
        self.assertGreaterEqual(len(jobs_response.data['jobs']), 1)

        exit_response = self.client.post(
            '/api/employee/exit-requests/',
            {
        ...
        self.assertEqual(ExitRequest.objects.filter(employee=self.employee).count(), 1)

    def test_employee_leave_types_sync_with_hr_policies(self):
        from people.models import LeaveType

        LeaveType.objects.create(name='Casual Leave', annual_quota=8, is_active=True)
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.employee_token.key}')
        response = self.client.get('/api/employee/leave-types/')
        self.assertEqual(response.status_code, 200)
        names = [item['name'] for item in response.data['leave_types']]
        self.assertIn('Casual Leave', names)

        invalid_leave = self.client.post(
            '/api/employee/leaves/',
            {
                'leave_type': 'Unknown Leave',
        ...
        self.assertEqual(invalid_leave.status_code, 400)

    def test_hr_can_manage_employee_documents_and_employee_sees_own_only(self):
        ...
        self.client.post(
            '/api/admin/hr/documents/',
            {
                ...
                'title': 'Other Employee ID Proof',
                'category': 'id_proof',
                'file_name': 'id.pdf',
                'file_data': 'secret-other-data',
            },
            format='json',
        )

        list_response = self.client.get(
            f'/api/admin/hr/documents/?employee_id={self.employee.id}'
        )
        self.assertEqual(list_response.status_code, 200)
        self.assertEqual(len(list_response.data['documents']), 1)

        patch_response = self.client.patch(
            f'/api/admin/hr/documents/{document_id}/',
            {'title': 'Updated Offer Letter', 'file_data': 'updated-offer-data'},
            format='json',
        )
        self.assertEqual(patch_response.status_code, 200)
        self.assertEqual(patch_response.data['document']['title'], 'Updated Offer Letter')

        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.employee_token.key}')
        employee_docs = self.client.get('/api/employee/documents/')
        self.assertEqual(employee_docs.status_code, 200)
        self.assertEqual(len(employee_docs.data['documents']), 1)
        self.assertEqual(employee_docs.data['documents'][0]['title'], 'Updated Offer Letter')
        self.assertNotIn('employee', employee_docs.data['documents'][0])

        blocked = self.client.get('/api/admin/hr/documents/')
        self.assertEqual(blocked.status_code, 403)

        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.hr_token.key}')
        archive_response = self.client.delete(f'/api/admin/hr/documents/{document_id}/')
        self.assertEqual(archive_response.status_code, 200)
        self.assertTrue(
            EmployeeDocument.objects.filter(pk=document_id, is_archived=True).exists()
        )

        self.client.credentials(HTTP_AUTHORIZATION=f'Token {self.employee_token.key}')
        after_archive = self.client.get('/api/employee/documents/')
        self.assertEqual(after_archive.status_code, 200)
        self.assertEqual(len(after_archive.data['documents']), 0)
